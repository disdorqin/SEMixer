...
def main(seed_cur, pred_l, data_type='ETTh1'):
    ...
    args.root_path = os.path.join(script_dir, 'dataset') + os.sep
    args.data_type=data_type  # 使用传入的数据集类型
    args.model='SEMixer'
    args.reduce_dim = 64
    ...
    args.Self_Attention_Mechanism = args.self_attn
    ...
    if args.data_type=='ETTh1':
        ...
        args.learning_rate=0.0001
        args.train_epochs = 50  # Traffic 专用 50 个 epoch
        if args.pred_len == 96 or args.pred_len == 192 or args.pred_len == 336 or args.pred_len == 720:
            args.seq_len = 2048
        args.stride=int(args.seq_len / args.maximum_patch_num)
        args.patch_len=int(args.seq_len / args.maximum_patch_num)*2
...
if __name__ == "__main__":
    ...
    # 数据集难度分类
    datasets_easy = ['ETTh1', 'ETTh2']  # 简单：变量少，数据量小
    datasets_medium = ['ETTm1', 'ETTm2', 'exchange_rate', 'national_illness']  # 中等
    datasets_hard = ['weather', 'electricity', 'traffic', 'solar_AL']  # 困难：变量多，耗时长
    
    Seeds_All = [0, 1, 2, 3, 4]
    pred_len = [96, 192, 336, 720]
    
    # 检查命令行参数 - 在 argparse 之前提取 mode
    if len(sys.argv) > 1 and sys.argv[1] in ['easy', 'medium', 'hard', 'all']:
        mode = sys.argv[1]
        # 移除 mode 参数，避免 argparse 报错
        sys.argv = [sys.argv[0]]
    else:
        mode = 'easy'
        sys.argv = [sys.argv[0]]
    ...
    if mode == 'easy':
        datasets = datasets_easy
    elif mode == 'medium':
        datasets = datasets_medium
    elif mode == 'hard':
        datasets = datasets_hard
    elif mode == 'all':
        datasets = datasets_easy + datasets_medium + datasets_hard
    else:
        datasets = datasets_easy
    ...
    print(f"随机种子：{Seeds_All}")
    print(f"总训练次数：{len(datasets)} x {len(pred_len)} x {len(Seeds_All)} = {len(datasets)*len(pred_len)*len(Seeds_All)} 次")
    print("="*60)
    
    for data_type in datasets:
        print(f"\n{'='*60}")
        print(f"开始数据集：{data_type}")
        print(f"{'='*60}\n")
        
        for seed in Seeds_All:
            for pred_l in pred_len:
                print(f"  >>> 运行 {data_type} | seed={seed} | pred_len={pred_l}")
                main(seed, pred_l, data_type)
        
        print(f"\n{'='*60}")
        print(f"数据集 {data_type} 完成!")
        print(f"{'='*60}\n")
